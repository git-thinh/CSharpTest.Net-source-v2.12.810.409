	...
	static partial class MessageQueue
	{
		...
		public static void Push(int depth, LogLevels level, Exception error, string format, object[] args)
		{
			try
			{
				...
				if((Configuration.LogOutput & LogOutputs.AspNetTrace) == LogOutputs.AspNetTrace
					&& System.Web.HttpContext.Current != null && System.Web.HttpContext.Current.Trace.IsEnabled
					&& (Configuration.LEVEL_ASPNET & data.Level) == data.Level)
				{// AspNet logging happens on this thread always
					try
					{
						if((data.Level & LogLevels.Warning) != LogLevels.None)
							System.Web.HttpContext.Current.Trace.Warn(data.MethodType, String.Format(Configuration.FormatProvider, Configuration.FORMAT_ASPNET, properties), data.Exception);
						else
